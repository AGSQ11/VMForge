...
class StorageController {
    public function index() {
        ...
        $html = '<div class="card"><h2>Storage Pools</h2><table class="table"><thead><tr><th>ID</th><th>Name</th><th>Driver</th><th>Config</th></tr></thead><tbody>'.$rows.'</tbody></table></div>';
        $html .= '<div class="card"><h3>Add Pool</h3>
        <form method="post" action="/admin/storage">
            <input type="hidden" name="csrf" value="'.$csrf.'">
            <input name="name" placeholder="name" required>
            <select name="driver"><option value="qcow2">qcow2</option><option value="lvmthin">lvmthin</option><option value="zfs">zfs</option></select>
    ...
    public function store() {
        ...
        $cfg = $_POST['config'] ? json_decode($_POST['config'], true) : null;
